...
if __name__ == "__main__":
    ...
    utils.plot_loss(train_history["loss"],
                    "Task 2 Model - More hidden units", npoints_to_average=10)
   # utils.plot_loss(
        #train_history_no_shuffle["loss"], "Task 2 Model - Normal", npoints_to_average=10)
    plt.ylim([0, .4])
    plt.subplot(1, 2, 2)
    plt.ylim([0.85, 1])
    utils.plot_loss(val_history["accuracy"], "Task 2 Model - Normal")
   # utils.plot_loss(
       # val_history_no_shuffle["accuracy"], "Task 2 Model - More hidden units")
    plt.ylabel("Validation Accuracy")
    plt.legend()
    plt.savefig("task4_train_loss_more_hidden_layers.png")
