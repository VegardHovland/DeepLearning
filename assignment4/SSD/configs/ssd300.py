...
train = dict(
    batch_size=16, #6/10 *epoker for 6000 interasjoneer
    amp=True,  # Automatic mixed precision
    log_interval=20, 
    seed=0,
    epochs=32, # 32 for 10k iterasjoner
...
model = L(SSD300)(
    ...
    num_classes=10 + 1  # Add 1 for background # Her som er feilen??
)
...
optimizer = L(torch.optim.Adam)(
    # Tip: Scale the learning rate by batch size! 2.6e-3 is set for a batch size of 32. use 2*2.6e-3 if you use 64
     lr=5e-6,  weight_decay=0.0005, amsgrad = True              #momentum=0.9
)
...
label_map = {
    0: "background",
    **{i + 1: str(i) for i in range(10)}        # kan endre index her?
}
