...
def calculate_accuracy(X: np.ndarray, targets: np.ndarray, model: BinaryModel) -> float:
    ...
    Args:
        ...
        targets: labelSs/targets of each image of shape: [batch size, 1]
        model: model of class BinaryModel
    Returns:
        Accuracy (float)
    """
    # TODO Implement this function (Task 2c)
    # accuracy = Num correct predictions / tot num predictions
    # num correct predictions = ?
    # tot num predictions = ?
    pred = np.round(model.forward(X))
    corr_pred = np.sum(pred == targets)
    acc = corr_pred/len(targets)  # Accuracy = (correct predictions / nr. of predictions)
    # accuracy = 0.0
    return acc
...
class LogisticTrainer(BaseTrainer):
    def train_step(self, X_batch: np.ndarray, Y_batch: np.ndarray):
        ...
        #Forward
        #backward
        X = model.forward(X_batch)
        model.backward(X_batch, X, Y_batch)
        # Perform gradient descent step here
