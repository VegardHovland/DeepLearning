...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray):
    ...
    for i in range(targets.shape[0]):
        for curr_y, curr_y_hat in zip(targets[i], outputs[i]):
            total += curr_y*np.log(curr_y_hat)
...
class SoftmaxModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        wx = X.dot(self.w)
        exp = np.exp(wx)
        y = np.divide(exp , np.array([np.sum(exp,axis=1)]).T)
        return y
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        # DO backward eq 8 Task 3a
        # self.grad = - (np.dot(X.T, (targets - outputs))) / len(targets)
        # Task 4b
        self.grad = (- (np.dot(X.T, (targets - outputs))) / len(targets) + self.l2_reg_lambda*self.w)
