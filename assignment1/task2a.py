...
def pre_process_images(X: np.ndarray):
    ...
    X = np.divide(X, (255/2)) - 1             # Convert to range -1 to 1
    X = np.hstack((X,bias))                   # Append collumn vector of ones to X
    return X
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    # y target
    # y hat outputs
    # def safe_log(x): return 0 if x == 0 else np.log(x)  # Kan gi problem med ln(0)??
    total = 0
    for curr_y, curr_y_hat in zip(targets, outputs):
        total += (curr_y * np.log(curr_y_hat) + (1 - curr_y) * np.log(1 - curr_y_hat))
...
class BinaryModel:
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        self.grad = - (np.matmul(X.T, (targets - outputs))) / len(targets)
